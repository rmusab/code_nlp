...
from my_wmd import WordEmbedding
...
def initialize_wmd_model(voc, w):
    ...
    wmd_model = WordEmbedding(model=word_embeds)
    return wmd_model


#wmd_model.wmdistance(label1, label2)


def find_closest_labels(label, tags, wmd_model, closest_num=10):
    distances = [ (j, wmd_model.wmdistance(label, tags[j].split())[0]) for j in range(len(tags)) ]
...
def main(args):
    if args.build:
        ...
        find_closest_labels(args.input.split(), tags, wmd_model, args.closest_num)
    elif args.labeld:
        voc = read_tokens()  # load the vocabulary
        w = load_embedding_mtx()  # load the embedding matrix
        wmd_model = initialize_wmd_model(voc, w)
        dist, T, _, _ = wmd_model.wmdistance(args.input1.split(), args.input2.split())
        print(dist)
        print(T)
    else:
